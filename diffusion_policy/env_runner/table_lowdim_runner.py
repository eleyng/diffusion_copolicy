...
from diffusion_policy.env.cooperative_transport.gym_table.envs.table_env import \
    TableEnv
...
class TableLowdimRunner(BaseLowdimRunner):
    def __init__(
        self,
        output_dir,
        n_train=10,
    ...
    def run(self, policy: BaseLowdimPolicy):
        ...
        for chunk_idx in range(n_chunks):
            ...
            obs = env.reset()
            env.ep = self.env_ep
            past_action = np.zeros(obs.shape)[..., :4]
            policy.reset()

            pbar = tqdm.tqdm(
                total=self.max_steps,
                desc=f"Eval TableLowdimRunner {chunk_idx+1}/{n_chunks}",
                leave=False,
